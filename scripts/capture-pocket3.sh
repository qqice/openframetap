...
case "$operation" in
  ...
  rtmp-proposal|rtmp-wifi-proposal)
    ...
    OPENFRAMETAP_USER_INITIATED=1 "$PYTHON_BIN" -m openframetap pocket3 rtmp send-approved \
      ...
      2>&1 | tee "$session_output"
    ;;
  rtmp-prepare-recovery)
    [[ -t 0 || "${OPENFRAMETAP_TEST_MODE:-0}" == "1" ]] || {
      echo 'RTMP prepare recovery requires the owner at an interactive TTY' >&2
      exit 4
    }
    [[ -f "$proposal_path" && -f "$workflow_state_path" ]] || {
      echo 'fixed recovery proposal or workflow state file is missing' >&2
      exit 2
    }
