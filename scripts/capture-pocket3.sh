...
[[ "$operation" == "listen" || "$operation" == "telemetry" || "$operation" == "pair-status" || "$operation" == "manual-frame" || "$operation" == "manual-pair-session" || "$operation" == "experiment" || "$operation" == "rtmp-proposal" || "$operation" == "rtmp-wifi-proposal" ]] || {
...
mkdir -p "$output_dir"
[[ "$operation" == "rtmp-proposal" || "$operation" == "rtmp-wifi-proposal" ]] && chmod 700 "$output_dir"
...
finalize_checksums() {
  if [[ "$operation" != "experiment" && "$operation" != "rtmp-proposal" && "$operation" != "rtmp-wifi-proposal" ]]; then
    return 0
  fi
...
case "$operation" in
  ...
  experiment)
    ...
    ;;
  rtmp-proposal|rtmp-wifi-proposal)
    [[ -t 0 || "${OPENFRAMETAP_TEST_MODE:-0}" == "1" ]] || {
