...
| Configure live stream | App `02` -> video `08`, `40/08/78` | fixed quality fields + packed RTMP URL | node-osmo and Moblin agree on Pocket 3 fixed byte `0x2E`; djictl agrees on the rest of the layout but uses `0x2A` and a separate start request. | Medium; fixed-byte and flow conflict | Private one-send candidate uses 720p30/4000 Kbps, sequence `0x8C2C`, and SHA-256 `0765f446...1a7fc4`. It is offline-validated and unsent; URL/key remain private. |
| Start transport stage | App `02` -> video `08`, `40/02/8E` | `01 01 1A 00 01 01` | djictl sends it after `08/78` and includes exact frame bytes; node-osmo/Moblin contain the same payload as a confirmation/start message but use it conditionally. | Medium; payload agreement, flow conflict | One exact candidate `551304030208b4bb40028e01011a0001013238` is allowlisted by SHA only after local `08/78` returned `D6` and produced no RTMP publisher. Not yet sent. |
