...
| Connect Wi-Fi | App `02` -> Wi-Fi `07`, `40/07/47` | packed SSID + packed PSK | node-osmo, Moblin, and djictl agree on request packing. Public/reference response lengths conflict. | High request schema; medium response schema | The first owner send, performed without Stage 2, produced no `0x8C19`, component `0x07`, or ACK frame and no router client. A fresh `0x8C1A` one-send proposal is now gated behind both hardware-validated prepare responses in one BLE connection; it remains unsent. |
