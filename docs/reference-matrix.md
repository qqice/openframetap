...
| Prepare transport stage2 | App `02` -> video `08`, `40/02/8E` | `00 01 1C 00` | djictl and a public Pocket 3 Mimo request capture agree byte-for-byte; the capture also has a same-sequence `80/02/8E` notification. node-osmo/Moblin omit it but use a separate cleanup stop. | High for this Pocket request/response exchange; trailing response semantics unknown | Hardware-validated: after exact Stage 1 ACK, one owner-invoked request returned same-sequence `80/02/8E` payload `0000011C0009030900000000000020` in approximately 113.5 ms. Generic `02/8E` remains denied. |
| Connect Wi-Fi | App `02` -> Wi-Fi `07`, `40/07/47` | packed SSID + packed PSK | node-osmo, Moblin, and djictl agree on request packing. Public/reference response lengths conflict between two and three zero bytes. | High for request and two-byte success form on this Pocket | Hardware-validated: the first send without Stage 2 had no response; a fresh `0x8C1A` send gated behind both prepare responses returned exact same-sequence `C0/07/47 payload 0000` after approximately 3.890 seconds. Assigned LAN IP and TCP reachability remain unobserved. |
| Configure live stream | App `02` -> video `08`, `40/08/78` | fixed quality fields + packed RTMP URL | node-osmo and Moblin agree on Pocket 3 fixed byte `0x2E`; djictl agrees on the rest of the layout but uses `0x2A` and a separate start request. | High for the `0x2E` frame inside the successful chain; exact standalone role remains unresolved | The fixed 720p30/4000 Kbps frame, sequence `0x8C2C`, SHA-256 `0765f446...1a7fc4`, returned exact `C0/08/78 payload 00` when sent after prepare and Wi-Fi in the same BLE connection. The later sample was 1280x720 H.264. URL/key remain private. |
| Start transport stage | App `02` -> video `08`, `40/02/8E` | `01 01 1A 00 01 01` | djictl sends it after `08/78` and includes exact frame bytes; node-osmo/Moblin contain the same payload as a confirmation/start message but use it conditionally. | High for the five-stage Pocket 3 chain; standalone necessity remains unresolved | The exact frame `551304030208b4bb40028e01011a0001013238` returned `C0/02/8E payload 00`; two seconds later MediaMTX reported a Pocket-side publisher with H.264 and AAC. Separate-connection attempts had returned `D6`, confirming the importance of preserving the session chain. |
