...
| --- | --- | --- | --- |
| Header magic is `0x55`; the 10-bit length includes header, payload, and CRC16; usual version is 1. | `djictl` message encoder/parser, `lib-osmo-ble` DUML builder, and the reverse-engineering dissector agree. Four published notification frames decode to their exact captured lengths. | High | Locally validated on 7,249 live FFF4 frames across the two passive sessions and final paired session. |
| CRC8 uses polynomial `0x31`, initial `0xEE`, reflected input/output, no xor-out. | `djictl` and `lib-osmo-ble` independently expose the same parameters; published frames validate byte-for-byte. | High | All 7,249 local live frames validated. |
| CRC16 uses polynomial `0x1021`, initial `0x496C`, reflected input/output, no xor-out; result is stored little-endian. | `djictl`, `lib-osmo-ble`, `node-osmo`, and Moblin agree; published frames validate exact trailing values. | High | All 7,249 local live frames validated. |
...
| `0x000d02` battery/status candidate | Published payloads contain plausible values 88 and 100 at offset 20. `node-osmo` and Moblin use that offset; `lib-osmo-ble` instead uses offset 0. The paired local session produced 65 frames with value 100 at that offset. | Medium | Decode offset 20 only when payload length is at least 21 and value is 0–100; retain the full payload. A screen correlation is still required before calling it verified battery percentage. |
