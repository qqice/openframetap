...
| Prepare transport stage2 | App `02` -> video `08`, `40/02/8E` | `00 01 1C 00` | djictl and a public Pocket 3 Mimo request capture agree byte-for-byte; the capture also has a same-sequence `80/02/8E` notification. node-osmo/Moblin omit it but use a separate cleanup stop. | Medium-high for wire frame; flow conflict remains | Exact recovery candidate SHA-256 `624c92dc...999286` is owner-approved but remains unsent. Generic send stays denied; one fixed wrapper may send it once only after the exact same-connection `02/E1` recovery ACK. |
| Connect Wi-Fi | App `02` -> Wi-Fi `07`, `40/07/47` | packed SSID + packed PSK | node-osmo, Moblin, and djictl agree on request packing. Public/reference response lengths conflict. | High request schema; medium response schema | Sent once by the owner as three ATT chunks. The following 33.070-second capture contained no `0x8C19`, component `0x07`, or ACK frame. Association result remains unknown; retry is prohibited. |
