...
Only the repository/runtime boundary, read-only platform audit, BlueZ advertisement transport, and metadata-only GATT enumeration exist in this milestone. The lower layers are roadmap items, not implemented capabilities.

## Layer boundaries

- `transport`: BlueZ/Bleak discovery, connection lifecycle, advertisement preservation, and GATT metadata enumeration.
- `protocol`: future DUML framing and validated command/response handling; it must not own model selection.
...
- `application state`: connection, telemetry, video, recording, and control state machines.
...
The first milestone performs no pairing, GATT writes, value reads, notification subscriptions, Wi-Fi provisioning, AP/P2P setup, video, camera commands, or gimbal movement. A later control layer must enforce command zeroing, maximum angular velocity/duration, and a 300–500 ms heartbeat loss stop before motion is enabled.
