...
4. **Current, missing prepare-stage recovery proposed:** the LAN-only user-space RTMP receiver and local publish/readback test are complete. The direct `02/E1 -> 07/47` attempt produced no Wi-Fi response or router client. Owner observations plus djictl and public Pocket 3 Mimo traffic identify omitted `02/8E payload 00011C00` as the strongest cause. A two-prompt same-connection recovery proposal is offline and unsent; Wi-Fi retry and RTMP configuration remain excluded.
