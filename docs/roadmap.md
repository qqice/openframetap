...
6. **Physical evidence captured; method gate still open:** three independent Pocket 4P 240 fps recordings produced exploratory low-latency/stable/aggressive distributions and retained `low-latency` as the conservative default. Colour rolling-shutter bands limited raw Gray decoding to 10.99–32.80%, below the predeclared 90% gate, so a cleaner repeat recording is required before claiming a final method-valid selection.
